        ...
        <div class="card-header">
          <div class="d-flex justify-content-between align-items-center">
            <header>
              ...
              <small class="text-muted">Total Aset Masuk (Pengeluaran Kas): Rp. {{ number_format($expenditure, 0, ',', '.') }}</small>
            </header>

            <div class="d-flex flex-column align-items-end gap-2">
              @if (auth()->user()->role->name == 'admin')
                <div class="d-flex align-items-center gap-2">
                  <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addAssetModal">Tambah Aset
                    Baru</button>

                  <a href="{{ route('asset.exportExcel') }}" class="btn btn-success ms-2" role="button">Export to Excel</a>
                  <a href="{{ route('asset.exportPdf') }}" class="btn btn-secondary ms-2" role="button">Export to PDF</a>
                </div>
              @endif
                        ...
                        <td class="text-nowrap">
                          <div class="dropdown dropup">
                            <button class="btn btn-sm btn-secondary dropdown-toggle"
                              id="dropdownMenuButton-{{ $asset->id }}" type="button" data-bs-toggle="dropdown"
                              aria-expanded="false">
                              <i class="bi bi-three-dots-vertical"></i>
                            </button>

                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton-{{ $asset->id }}">
                              {{-- <li><a href="#" class="dropdown-item" data-bs-toggle="modal"
                                  data-bs-target="#editAssetModal"
                                  onclick="editAsset({{ json_encode($asset) }})">Ubah</a></li> --}}
        ...
        <form id="editAssetForm" method="POST">
          ...
          <div class="modal-body">
            <div class="mb-3">
              ...
              <input type="text" name="name" id="edit_name" class="form-control" required="required" autofocus>
            </div>
            ...
            <div class="mb-3">
              ...
              <input type="date" name="date" id="edit_date" class="form-control" required="required">
            </div>

            <div class="mb-3">
              <label for="edit_amount" class="form-label">Jumlah</label>
              <input type="number" class="form-control" id="edit_amount" name="amount" required="required"></input>
            </div>

            <div class="mb-3">
              <label for="edit_value" class="form-label">Nilai</label>
              <input type="number" name="value" id="edit_value" class="form-control" required="required">
            </div>
          </div>
